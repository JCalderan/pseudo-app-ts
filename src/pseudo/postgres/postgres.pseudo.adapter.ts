...
import {
  ...
  FailedTransaction,
  FinishedTransaction,
} from 'src/pseudo/domain/pseudoAdapter';
...
export class PostgreSQLPseudoAdapter implements PseudoAdapter {
  ...
  async find(pseudo: Pseudo): Promise<Pseudo> {
    try {
      const existingPSeudo: PseudoEntity = await this.pseudoRepository.findOne({
        name: pseudo.getName(),
      });
      return existingPSeudo !== undefined
        ? Pseudo.of(existingPSeudo.name)
        : undefined;
    } catch (error) {
      // TODO: use logger
  ...
  async save(pseudo: Pseudo): Promise<Pseudo> {
    try {
      const pseudoToSave: PseudoEntity = this.pseudoRepository.create({
        name: pseudo.getName(),
      });
