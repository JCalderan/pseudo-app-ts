...
const mockPseudoAdatper = (
  ...
  findLastRegisteredPseudoFn: () => Promise<Pseudo>,
): PseudoAdapter => {
  return {
...
describe('PseudoService', () => {
  describe('Pseudo.registerPseudo', () => {
    it('should return a Pseudo from a valid input string', async () => {
      ...
      const pseudoService: PseudoService = new PseudoService(
        aPseudoAdapterThatSaveAnyPseudo,
      );

      // When
      const aSavedPseudo: Pseudo = await pseudoService.registerPseudo(
        aStringValue,
      );
    ...
    it('should return a succeeded GuardedValue<Pseudo> from an already taken input string', async () => {
      ...
      const aPseudoAdapterThatResolveAnUnavailablePseudo: PseudoAdapter =
        mockPseudoAdatper(
          (_) => Promise.resolve(_),
      ...
      const pseudoService: PseudoService = new PseudoService(
        aPseudoAdapterThatResolveAnUnavailablePseudo,
      );
    ...
    it('should throw an Error when creating a Pseudo from an invalid input String', async () => {
      ...
      // When
      const pseudoRegistration = () =>
        pseudoService.registerPseudo(aStringValue);
